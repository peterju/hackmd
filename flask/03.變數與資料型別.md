...
#### 字串的切片(slicing)
切片即取得部分字串，Python 的序列型別如同一般程式語言的陣列，可用【索引】指定元素的位置(位置從 0 開始)，其切片語法為 `[n:m]` 代表從字串索引第 n 的位置取到 m-1 的位置。
```
>>> x = "This is a book."
...
【序列結構】的型別，元素以【逗點】隔開，索引從 0 開始，皆無需宣告型別與大小
- list(串列)：序列結構，以中括號包裹，元素可變更
- tuple(元組)：序列結構，以小括號包裹，元素不可變更
...
# 以索引取用(與字串相同概念)
print(garagelist[0])                # Benz
print(garagetuple[1])               # Toyota
print(garagetuple[-1])              # Nissan
print(garagetuple[-2])              # Toyota
# print(garagelist[3])              # 超出索引範圍

# 切片取用(與字串相同概念)
print(garagelist[1:3])              # 從註標 1 開始取到註標 2
print(garagelist[:2])               # 從註標 0 開始取到註標 1
print(garagetuple[-2:])             # 從註標 -2 開始到最後

# 迴圈取用
for eachcar in garagelist:          # for 無須括弧用冒號取代，區塊內容須縮排
    print(eachcar)
...
print(" > ".join(garagetuple))      # 以 > 作為元素 join 在一起的分隔符號

# 異動元素
garagelist[0] = "Bentley"           # 索引更改 list 元素值
garagelist[1:3] = ["Audi", "Lexus"] # 切片更改 list 元素值
print(garagelist)

# 更改 list 元素值並提示輸入其他廠牌
garagelist[2] = input("Please input your car brand : ")
print(garagelist)
# garagetuple[2]="Mitsubishi"       # 更改 tuple 元素值，但會失敗

garagelist.append("Porsche")        # 增加 list 元素值
print(garagelist)
# 將一個已存在的 list 或 tuple 增加至目前的 list 中
garagelist.extend(garagetuple)
print(garagelist)

double_list = garagelist+garagelist # 串聯二個 list
print(double_list)
...
print(garagelist)
print(newlist.pop())                # 刪除並回傳 list 中最後一個元素
print(garagelist)
print(newlist.pop(1))               # 根據註標刪除並回傳刪除的元素
print(garagelist)
print(len(garagelist))              # 取得 list 的元素個數
...
garagelist.sort()                   # list 遞增排序(預設)
print(garagelist)
garagelist.sort(reverse=True)       # list 遞減排序
print(garagelist)
garagelist.sort(key=cmp_len)        # list 自訂排序
print(garagelist)
sortedList = sorted(garagelist)     # 不更動原 list 排序回傳排序的新 list
print(sortedList)
...
### set(集合)
set 物件是無序的集合(unordered collection)，建立時與 dict 一樣使用大括弧，set 很重要的精髓是 set 不會包含重複的資料。
...
s.add(7)
s.add(7)
print(s)          #集合不會將重複的元素加入
s.add('x')
print(s)
if 'x' in s:        #測試某元素是否在 set 裡
    s.remove('x')
print(s)
```
...
print('交集')
# 集合的交集運算
print(fruits1 & fruits2)                     #使用 & 符號取交集
print(fruits2.intersection(fruits1))    #使用集合的intersection方法取交集
print('聯集')
# 集合的聯集運算
print(fruits2 | fruits1)                      #使用 & 符號取聯集
print(fruits1.union(fruits2))              #使用集合的union方法取聯集
print('差集')
# 集合的差集運算(請留意集合的位置)
print(fruits1 - fruits2)                       #使用 - 符號取差集
print(fruits1.difference(fruits2))       #使用集合的difference方法取差集
print(fruits2 - fruits1)                       #使用 - 符號取差集
print(fruits2.difference(fruits1))       #使用集合的difference方法取差集
print('對稱差集')
# 集合的對稱差集運算(取出2集合不重複的元素)
print(fruits2 ^ fruits1)                      #使用 ^ 符號取對稱差集
print(fruits1.symmetric_difference(fruits2))  #使用集合的symmetric_difference方法取對稱差集
```
### NoneType
- 沒有傳回值的函數，預設會傳回 None
